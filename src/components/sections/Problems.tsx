import { AlertCircle, Bug, FileWarning, TrendingDown } from "lucide-react";
...
const Problems = () => {
  return (
    <section className="bg-surface pt-20 pb-10 md:pt-28 md:pb-14">
      <div className="section-container">
        {/* Section Header */}
        <div className="mb-12 max-w-2xl">
          <p className="mb-2 text-sm font-medium uppercase tracking-wider text-primary">Masalah Umum</p>
          <h2 className="mb-4 text-3xl font-bold text-foreground sm:text-4xl">Pernah mengalami ini?</h2>
          <p className="text-lg text-muted-foreground">Banyak bisnis menghadapi kendala serupa. Kabar baiknya, semua ini bisa diatasi dengan sistem yang tepat.</p>
        </div>

        {/* Problems Grid */}
        <div className="grid gap-6 sm:grid-cols-2">
          {problems.map((problem, index) => (
            <div key={problem.title} className="group rounded-xl border border-border bg-background p-6 transition-all duration-300 hover:border-primary/20 hover:shadow-lg" style={{ animationDelay: `${index * 0.1}s` }}>
